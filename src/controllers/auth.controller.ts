...
export class AuthController {
  public async register(req: Request, res: Response) {
    try {
      ...
      const existingUser = await UserService.findOneByEmail(
      ...
      validatedInput.password = await AuthService.hashPassword(
  ...
  public async verifyPasswordResetToken(req: Request, res: Response) {
    try {
      ...
      const tokenJwt = (await AuthService.verifyPasswordResetToken(
        reset_token
      )) as JwtPayload
  ...
  public async resetPassword(req: Request, res: Response) {
    try {
      const { new_password, reset_token } = validateResetPasswordParams(
        req.body,
        req.query
      )
      const token = await TokenService.findOneByCode(reset_token)
  ...
  public async generateAccessToken(req: Request, res: Response) {
    try {
      ...
      const tokenJwt = (await AuthService.verifyRefreshToken(
        refresh_token
      )) as JwtPayload
