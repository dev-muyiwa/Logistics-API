...
export class AuthService {
    ...
    public static generateRefreshToken(id: string, email: string) {
        return jwt.sign({id: id, email: email, type: 'refresh'}, config.USER_REFRESH_SECRET, {
            expiresIn: '4d',
            issuer: 'Moyosore'
        })
    }
    ...
    public static generatePasswordResetToken(id: string, email: string) {
        return jwt.sign({id: id, email: email, type: 'reset'}, config.USER_RESET_SECRET, {
            expiresIn: '30m',
            issuer: 'Moyosore'
        })
    }
    ...
    public static generateAccessToken(id: string, email: string) {
        return jwt.sign({id: id, email: email, type: 'access'}, config.USER_ACCESS_SECRET, {expiresIn: '1h', issuer: 'Moyosore'})
    }
