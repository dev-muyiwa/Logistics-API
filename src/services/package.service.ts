...
export class PackageService {
    public static async createPackage(data: Package) {
        const {name, description, primary_email, secondary_email, user_id, pickup_date} = data
        const query =
            `INSERT INTO packages (id, name, description, pickup_date, primary_email, secondary_email, tracking_code,
                                   user_id, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
             RETURNING *`
        const result = await pgClient.query(query, [ulid(), name, description, pickup_date, primary_email, secondary_email, ulid(), user_id, new Date()])
    ...
    public static async getPackagesForAUser(id: string, page: number) {
        ...
        const paginatedQuery =
            `SELECT * FROM packages WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`
        const countQuery =
            `SELECT COUNT(*) AS total_count FROM packages WHERE user_id = $1`
        try {
            await pgClient.query('BEGIN')

            const paginatedResult = await pgClient.query(paginatedQuery, [id, limit, offset])
            const countResult = await pgClient.query(countQuery, [id])

            await pgClient.query('COMMIT')

            return {data: paginatedResult.rows, total_count: countResult.rows[0].total_count}
        } catch (err) {
