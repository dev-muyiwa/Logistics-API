...
export function successResponse(res: Response, data: any, message: string, code: number = 200): Response {
    return res
        .status(code)
        .json({success: true, data: data, message: message})
}

export function errorResponse(res: Response, err: any, message?: string, code: number = Exception.BAD_REQUEST): Response {
