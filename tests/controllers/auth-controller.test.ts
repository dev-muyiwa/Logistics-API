...
describe('AuthController', () => {
  ...
  describe('register', () => {
    it('should create a new user', async () => {
      ...
      ;(AuthService.generateRefreshToken as jest.Mock).mockReturnValueOnce(
        'refreshToken'
      )
      ...
      expect(UserService.findOneByEmail as jest.Mock).toHaveBeenCalledWith(
        'test@example.com'
      )
      expect(AuthService.hashPassword as jest.Mock).toHaveBeenCalledWith(
        'password'
      )
    ...
    it('should throw an error if email already exists', async () => {
      ;(UserService.findOneByEmail as jest.Mock).mockResolvedValueOnce({
        email: 'test@example.com'
      })
  ...
  describe('login', () => {
    ...
    it("should return and error if the email wasn't found", async () => {
      ...
      expect(
        AuthService.generateAccessToken as jest.Mock
  ...
  describe('forgotPassword', () => {
    it('should send a password reset email for an existing user', async () => {
      ...
      ;(
        AuthService.generatePasswordResetToken as jest.Mock
      ...
      expect(
        AuthService.generatePasswordResetToken as jest.Mock
      ).toHaveBeenCalledWith('user_id', 'test@example.com')
  ...
  describe('verifyPasswordResetToken', () => {
    it('should verify and update the password reset token', async () => {
      ...
      ;(
        AuthService.verifyPasswordResetToken as jest.Mock
      ).mockResolvedValueOnce(tokenJwt)
      ...
      expect(TokenService.findOneByCode as jest.Mock).toHaveBeenCalledWith(
        'reset_token'
      )
      expect(
        AuthService.verifyPasswordResetToken as jest.Mock
      ).toHaveBeenCalledWith('reset_token')
  ...
  describe('generateAccessToken', () => {
    it('should generate an access token for a valid refresh token', async () => {
      ...
      ;(AuthService.verifyRefreshToken as jest.Mock).mockReturnValueOnce({
        id: user.id,
        email: 'test@example.com'
      })
      ...
      expect(AuthService.verifyRefreshToken as jest.Mock).toHaveBeenCalledWith(
        'refresh_token'
      )
      ...
      expect(AuthService.generateAccessToken as jest.Mock).toHaveBeenCalledWith(
        'user_id',
        'test@example.com'
      )
